...
Разметка: короткие абзацы через пустую строку; списки строками с «- »; **жирным** только ключевые термины (1–3 на ответ). Другой маркдаун не используй.`;

  if (mode !== "review") return base;

  return `${base}

Ты разбираешь ошибки конкретной тренировки ученика (см. <ошибки_сессии>). Пройдись по каждой: почему ошибся, как рассуждать верно, дай мини-приём запоминания. Тон — поддерживающий, обращение на ты. Структура: по абзацу-два на ошибку, каждая начинается строкой «**Ошибка N: тема**»; между ошибками пустая строка; в конце одно предложение с предложением потренировать слабое место.`;
}
